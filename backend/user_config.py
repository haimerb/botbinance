...
def load_user_config(user_id: int) -> dict:
    ...
    config["stop_loss_pct"] = cfg.stop_loss_pct if cfg.stop_loss_pct is not None else STOP_LOSS_PCT
    config["take_profit_pct"] = cfg.take_profit_pct if cfg.take_profit_pct is not None else TAKE_PROFIT_PCT
    config["max_position_size"] = cfg.max_position_size if cfg.max_position_size is not None else MAX_POSITION_SIZE
    config["ma_fast_period"] = cfg.ma_fast_period if cfg.ma_fast_period is not None else MA_FAST_PERIOD
    config["ma_slow_period"] = cfg.ma_slow_period if cfg.ma_slow_period is not None else MA_SLOW_PERIOD
    config["ml_confidence_threshold"] = cfg.ml_confidence_threshold if cfg.ml_confidence_threshold is not None else ML_CONFIDENCE_THRESHOLD
