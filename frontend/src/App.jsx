...
const COMMON_SYMBOLS = [
  ...
  'THETAUSDT', 'VETUSDT', 'HBARUSDT', 'ICPUSDT', 'STXUSDT', 'RNDRUSDT',
]
...
function AccountSettings({ user, onBack }) {
  ...
  const handleAddSymbol = async () => {
    ...
    setConfig(updated)
    setNewSymbol('')
    setMessage(`Símbolo ${sym} agregado`)
    setTimeout(() => setMessage(''), 3000)
  }
  ...
  const handleRemoveSymbol = async (sym) => {
    ...
    setConfig(updated)
    setMessage(`Símbolo ${sym} eliminado`)
    setTimeout(() => setMessage(''), 3000)
  }
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <button type="button" className="btn-ghost" onClick={async () => {
                  try {
                    ...
                    setMessage('Parámetros sugeridos cargados. Revisa y guarda.')
                    setTimeout(() => setMessage(''), 3000)
                  } catch (e) {
                    setMessage(`Error: ${e.message}`)
                  }
...
function PriceMiniChart({ history }) {
...
function SymbolCard({ symbol, price, signal, position, active, onClick }) {
...
function Dashboard({ user, onLogout, onAccount }) {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      if (s?.current_prices) {
        ...
        Object.entries(s.current_prices).forEach(([sym, price]) => {
          if (price) {
            const arr = hist[sym] || []
  ...
  const handleStart = async () => { try { await api.startBot(); fetchAll() } catch (e) { alert(e.message) } }
  const handleStop = async () => { try { await api.stopBot(); fetchAll() } catch (e) { alert(e.message) } }
            ...
            <table>
              ...
              <tbody>
                {[...trades].reverse().map((t, i) => (
                  <tr key={i} className={t.pnl_pct != null ? (t.pnl_pct >= 0 ? 'row-win' : 'row-loss') : ''}>
